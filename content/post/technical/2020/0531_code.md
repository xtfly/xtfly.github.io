...
<!--more-->
回起曾经走读代码见过如下的写法（`写法三`，还是以getRejectMessageCount实现为例）：
...
回到案例的代码，再来对比说明一下
 ...
 - 写法二：1.8版本针对不存在则put操作场景提供更为简单的API，涉及到锁的可见性

# 线程安全

JDK提供多种容器、原子对象与线程池，让多线程编程变得较为简单，但简单也隐藏了复杂与其背后的原理，若使用不当，也会造成多线程安全问题。
...
再回到案例的代码：
 ...
 - 顺序性：涉及到Java中的指令重排序问题，案例代码不好直接说明此特征，简单说多个线程使用变量依赖是要有序的，不可被打断打乱，volatile变量则不可重排序
