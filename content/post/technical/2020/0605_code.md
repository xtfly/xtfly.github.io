...
写出ServiceFactory目的是想解决由于`@Autowired`只能用于Spring管理的Bean对象中，并不能用于其它对象中。而实现代码会在其它非Bean对象中随时调用XXXService，ServiceFactory则提供了获取Spring管理的XXXService一种变通途径。
...
为什么上面集中且容易察觉到的重复（相似），我们不去消除它。因为写重复代码太容易了，传统消除重复的套路（提取函数）在上面的代码已不再能有效解决，更别说满足`开闭原则`。若消除这类重复，而是需要我们对Spring的IoC运作机制较深入了解。
...
拿到ApplicationContext之后，我们就可以获取任一Bean对象了，可以消除了相似代码，也解决了违背`开闭原则`的问题。
...
通过采用List注入方式解决扩展性问题，那又怎么解决不同Interceptor注册先后问题？Spring还提供一个@Order注解，只需要在各个类声明时，给它排个序，那List中注入的对象就具有先后顺序：
...
Spring框架其中的IoC容器负责管理Bean对象的生命周期，其底层核心对象：
...
提供Spring的Bean管理，我们必定会提到两个词：DI与IoC。Spring是Bean的IoC容器，我们使用Bean时需要DI，初学者容易混淆他们。
...
前面提到的Spring知识也只是点为止，而市面上编程、框架介绍这类书籍都通常非常的厚，我们哪些时间去学习啊；甚至通篇大段的代码讲解，让我去学习失去新鲜感。我也想掌握更多的知识，把代码写得更简练。面对编程中的各个体系，我们怎么去学习？学习有个金字塔理论：
...
后面几种效果高的学习方式，都是团队学习、主动学习与参与式学习。结合编程应用上述理论：
  ...
  - 个人不建议平时花时间去看什么编程、框架之类的书籍，而是在工作中遇到问题，针对问题去主动寻求答案。正如案例代码一的写法，当你写一到三个对象注入时，可能不会想到优化。当写到七到八个时，那就应该想想有没有更好的办法。采取顺藤摸瓜式的思考，面向搜索学习：Spring怎么管理Bean->是否有Bean查询接口->我怎么拿到查询接口->是否有什么扩展点......刚开始可能完全没有背景知识毫无思路，但如果不走第一步，也将永远停留在原地。
  - 平时注意总结与分享，分享不一定要搞个正式会议，形式多样。当你去考虑如何教别人时，就会先去了解更多的相关知识，也会在总结中有更多的思考，在讲解的过程中有更深的体会。这样日累月累下来，将会有越来越多的收获。

在工作中 [不断学习](http://lanlingzi.cn/post/thoughts/2019/0511_study/)，将是程序员不断提升的不二法门。

# 结语

写出干净的代码，需要我们对所使用的框架机制有较深入的了解，一是可以避免出现框架已有机制没有应用起来出现重复或复杂的代码； 二是可以避免当出现问题时两手抓狂不知如何下手定位。在工作中带着问题，带着思考去学习。学习框架的运行机制，以及其应用的原理、设计原则，将会使自己从编码的纯体力劳动解放出来，发现更多有趣的新技术与技巧。并把这些应用到工作中，解决现实的问题，也将收获编程带来的满足感，快乐感。
