...
std::unique_ptr<A> getAWhitUniquePtr() {
    return std::make_unique<A>(param1, param2); // 大对象
}
```

# 解题

前面对RAII，所有权，智能指针，移动语义，以及RVO都没有展开来一一深入讲解，大家可以借助搜索来深入学习。这些知识点，只为了更好使用智能来减少内存泄露作为铺垫。现在C++在语义上已经可以实现半自动化的内存回收，unique_ptr+move：
