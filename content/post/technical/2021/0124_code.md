...
从上述代码中可以看出：
 ...
 - [4] `InitializeCb`是一个回调函数，它传递dispatcher参数，返回要存储的对象指针。主线程与工作线程实际存储的数据都是通过调用`InitializeCb`函数来拿到真实要存放的数据`ThreadLocalObjectSharedPtr`，对于同一个`InitializeCb`函数，会在不同的工作线程中调用多次，相当于对槽位数据进行拷贝
...
// [2]前面的thread_local_data_.data_类型如下
...
从上述代码中可以看出：
  ...
  - [3] `thread_local_data_` 是InstanceImpl的静态成员，它一个TLS对象。thread_local是C++11新引入，thread_local关键字修饰的变量具有线程周期(thread duration)，这些变量(或者说对象）在线程开始的时候被生成(allocated)，在线程结束的时候被销毁(deallocated)
...
Slot接口还提供两个函数来直接传递数据给所有注册的线程：
  ...
  - 相同点：机制与set函数实现类似，采用`dispatcher.post`把数据分发给各注册线程处理，通过回调`UpdateCb`函数来产生数据
  - 不同点：不会把`UpdateCb`函数产生数据更新到TLS；TLS中存储的数据适合于多次需要读取的场景，`runOnAllThreads`函数适合一次数据传递，不需要长期存储
...
SlotPtr InstanceImpl::allocateSlot() {
  ...
  const uint32_t idx = free_slot_indexes_.front(); // [3] 从空闲的队首取一个
